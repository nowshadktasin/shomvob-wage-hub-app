...
export const fetchTransactionHistory = async (phoneNumber: string, userAccessToken: string, rowId: string): Promise<TransactionHistoryResponse['data']> => {
  ...
  const url = `${apiUrl}?user_access_token=${userAccessToken}&phoneNumber=${formattedPhoneNumber}&rowId=${rowId}`;
